...
// be terminated.
type WalkFn func(s []byte, v *addr.Addr) bool
...
func (t *Tree) Add(s []byte, v *addr.Addr) (*addr.Addr, bool) {
	t.rw.Lock()
	defer t.rw.Unlock()
	var parent *node
	n := t.root
...
func (t *Tree) Delete(s []byte) (*addr.Addr, bool) {
	t.rw.Lock()
	defer t.rw.Unlock()
	var parent *node
	var label byte
...
func (t *Tree) deletePrefix(parent, n *node, prefix []byte) uint {
	...
	if len(prefix) == 0 {
		...
		recursiveWalk(n, func(s []byte, v *addr.Addr) bool {
			subTreeSize++
			return false
		})
		if n.isLeaf() {
...
func (t *Tree) Get(s []byte) (*addr.Addr, bool) {
	t.rw.RLock()
	defer t.rw.RUnlock()
	n := t.root
	search := s
...
func (t *Tree) LongestPrefix(s []byte) ([]byte, *addr.Addr, bool) {
...
func (t *Tree) FilterKey(fn WalkFn) (li [][]byte) {
	...
	recursiveWalk(t.root, func(s []byte, v *addr.Addr) bool {
		if fn(s, v) {
			li = append(li, s)
		}
		return true
